...
export default function useProjectsData(
  ...
  useEffect(() => {
    ...
    setProjects(
      appData.projects.map((projectIdentifier) => {
        return {
  ...
  useEffect(
    () =>
      setSyncs(
        projects.map((p) => {
          const setSyncState = (newState: SyncState<ProjectData>) =>
            setProjects(
          ...
          const fetch = createSyncFetch(syncState, setSyncState, () =>
            getProject({ projectId: p.id }),
          );
          if (p.status == SyncStatus.WAITING) fetch();
