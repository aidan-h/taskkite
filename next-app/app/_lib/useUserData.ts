...
import { AppData, ClientEvent, CreateTaskEvent, DeleteTaskEvent, EditTaskEvent, Project } from "./data";
import { IncrementalData, SyncClient, SyncStatus, useIncrementalData, useSyncClient } from "./sync";
...
export function useProjectData(
	...
	return useIncrementalData(
		...
		(events, data) =>
			syncProject({ projectId: data.id, changes: events, index: data.historyCount }),
		applyEvent,
	);
}
