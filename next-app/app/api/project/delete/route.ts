import { affectProjectSchema } from "@/app/_lib/schemas";
import { handleClientPostReq } from "@/app/_server/handleClient";

const DELETE_PROJECT_STATEMENT =
	"DELETE FROM project WHERE id = ? AND owner = ?";
...
export const POST = handleClientPostReq(
	...
	async (db, session, data) => {
		...
		try {
			await db.execute(DELETE_PROJECT_STATEMENT, [
				data.projectId,
				session.email,
			]);
