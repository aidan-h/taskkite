import { AffectProject, Label, Project, affectProjectSchema } from "@/app/_lib/data";
...
async function getLabels(db: Connection, projectId: number, id: number): Promise<Label[]> {
	const [rows] = await db.execute<RowDataPacket[]>(GET_LABELS_STATEMENT, [projectId, id])
