...
import { validateInputValue } from "@/app/_lib/form";
...
function Form({ id, close }: { id: number, close: () => void }) {
	...
	return <Formik validate={values => {
		...
		validateInputValue(descriptionSchema, values.description, errors, "description")
		return errors
	}

	}
		onSubmit={(values, { setSubmitting }) => {
			createTask(id, values.name, values.description).then(() => {
				update()
				close()
			}, console.error).finally(() => setSubmitting(false))
		}}
		initialValues={{ name: "Task name", description: "" }}>
		{({ handleSubmit, isSubmitting, handleChange, values, errors }) => <form onSubmit={handleSubmit}>
			<p>Name</p>
			<br />
			...
			<p>Description</p>
			<br />
			<input type="text" onChange={handleChange} value={values.description} name="description" />
			{errors.description}
			<br />
...
export default function TaskCreation({ id }: { id: number }) {
	...
	if (active)
		return <Form id={id} close={() => setActive(false)} />
